...
function closePopup() {
    ...
    setTimeout(() => {
        ...
        fetchOverlay.classList.remove("closing");
    },400);
}
...
// Load server list from config file first
async function loadServerConfig() {
    try {
        const res = await fetch('./config/servers.json', { cache: "no-store" });
        if (res.ok) {
            const config = await res.json();
            MASTER_LIST_URL = config.master_list_url || '';
            deployedServers = config.deployed_servers || [];
            addLog('Loaded server list from config file');
        }
    } catch {
        addLog('Config file not found, using default list');
        deployedServers = [
            "cxlvin-22d733-2tb4hqsf2a-uc.a.run.app",
            "cxlvin-2wd733-2tb4hqsf2a-uc.a.run.app",
            "cxlvin-q2d733-2tb4hqsf2a-uc.a.run.app",
            "cxlvin-629d3d-2tb4hqsf2a-uc.a.run.app",
            "cxlvin-397082-2tb4hqsf2a-uc.a.run.app",
            "cxlvin-07cab7-2tb4hqsf2a-uc.a.run.app"
        ];
    }
}
...
function updateLatencyStatus() {
    ...
    addLog(`System Status → ${currentLatency === 'good' ? 'STABLE' : currentLatency === 'medium' ? 'ELEVATED' : 'CRITICAL'}`);
}

function updateConfigOutput() {
    const selectedAddress = addressSelect.value;
    const selectedSni = sniSelect.value;
    const activeHostParam = cloudRunHost || selectedSni;

    if (!selectedTransport || !selectedProtocol || !activeHostParam) {
        configOutput.textContent = 'Select Transport, Protocol and Server Host to generate config';
        return;
    }

    let template = configTemplates[selectedTransport][selectedProtocol];

    if (selectedProtocol === 'vmess') {
        let vmessObj;
        if (selectedTransport === 'websocket') vmessObj = {...vmessBase.ws};
        if (selectedTransport === 'http-upgrade') vmessObj = {...vmessBase.hu};
        if (selectedTransport === 'xhttp') vmessObj = {...vmessBase.xh};
        vmessObj.add = selectedAddress;
        vmessObj.sni = selectedSni;
        vmessObj.host = activeHostParam;
        const encoded = btoa(JSON.stringify(vmessObj));
        template = template.replace('{{VMESS_WS}}', encoded).replace('{{VMESS_HU}}', encoded).replace('{{VMESS_XH}}', encoded);
    }

    const finalConfig = template
        .replace(/{{ADDRESS}}/g, selectedAddress)
        .replace(/{{SNI}}/g, selectedSni)
        .replace(/{{SELECTED_HOST}}/g, activeHostParam);

    configOutput.textContent = finalConfig;
}
...
async function loadAllRealData() {
    ...
    try {
        ...
        elIP.textContent = ipData.ip;
        elIP.classList.remove('status-wait');
        elIP.classList.add('status-high')
        addLog(`Public IP detected: ${ipData.ip}`);
    } catch (err) {
        elIP.textContent = 'Unavailable';
        addLog('Could not fetch public IP');
    }

    try {
        const visitorRes = await fetch('https://api.countapi.xyz/hit/cxlvin-server/visits');
        const visitorData = await visitorRes.json();
        elVisitor.textContent = visitorData.value;
        addLog(`Total visitors: ${visitorData.value}`);
    } catch (err) {
        elVisitor.textContent = '1';
        addLog('Visitor count offline');
    }
}
